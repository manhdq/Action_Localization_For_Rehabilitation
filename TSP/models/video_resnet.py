...
class BasicStem(nn.Sequential):
    ...
    def __init__(self, num_channels=3) -> None:
        super().__init__(
            nn.Conv3d(num_channels, 64, kernel_size=(3, 7, 7), stride=(1, 2, 2), padding=(1, 3, 3), bias=False),
            nn.BatchNorm3d(64),
            nn.ReLU(inplace=True),
        )
...
class R2Plus1dStem(nn.Sequential):
    ...
    def __init__(self, num_channels=3) -> None:
        super().__init__(
            nn.Conv3d(num_channels, 45, kernel_size=(1, 7, 7), stride=(1, 2, 2), padding=(0, 3, 3), bias=False),
            nn.BatchNorm3d(45),
            nn.ReLU(inplace=True),
...
class VideoResNetLite(nn.Module):
    def __init__(
        self,
        block: Type[Union[BasicBlock, Bottleneck]],
        conv_makers: Sequence[Type[Union[Conv3DSimple, Conv3DNoTemporal, Conv2Plus1D]]],
...
def _video_resnet_lite(arch: str, pretrained: bool = False, progress: bool = True, **kwargs: Any) -> VideoResNet:
    model = VideoResNetLite(**kwargs)
    return model
...
def r2plus1d_12(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> VideoResNet:
    """Constructor for the 12 layer deep R(2+1)D network as in
    https://arxiv.org/abs/1711.11248
    ...
    Returns:
        nn.Module: R(2+1)D-18 network
    """
    return _video_resnet_lite(
        "r2plus1d_12",
        pretrained,
        progress,
        block=BasicBlock,
        conv_makers=[Conv2Plus1D] * 2,
        layers=[2, 3],
        stem=R2Plus1dStem,
        **kwargs,
    )