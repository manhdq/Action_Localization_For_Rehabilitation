...
class ImageTransformer(object):
    ...
    def rotate_along_axis(self, theta=0, phi=0, gamma=0, dx=0, dy=0, dz=0):
        ...
        new_image = cv2.warpPerspective(self.image.copy(), mat, (self.width, self.height))

        new_kpts = np.ones((len(self.kpts), 3))
        new_kpts[:, :2] = self.kpts
        new_kpts = new_kpts @ mat.T
        new_kpts = new_kpts[:, :2] / new_kpts[:, 2:3]
        
        return new_image, new_kpts
