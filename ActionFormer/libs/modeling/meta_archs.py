...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        max_div_factor = 1
        # print(self.fpn_strides, self.mha_win_size)
    ...
    @torch.no_grad()
    def detect_from_features(self, features_infos):
        if not isinstance(features_infos, list):
            features_infos = [features_infos]

        batched_inputs, batched_masks = self.preprocessing_from_features(features_infos)
        ...
        vid_idxs = [x['video_id'] for x in features_infos]
        vid_fps = [x['fps'] for x in features_infos]
        vid_lens = [x['duration'] for x in features_infos]
        vid_ft_stride = [x['feat_stride'] for x in features_infos]
        vid_ft_nframes = [x['feat_num_frames'] for x in features_infos]
    ...
    @torch.no_grad()
    def preprocessing_from_features(self, features_infos, padding_val=0.0):
        """
            features (list[Tensor]):  each has shape (C, T)
        """
        features = [features_info['feats'] for features_info in features_infos]
        feats_lens = torch.as_tensor([feat.shape[-1] for feat in features])
        ...
        batched_inputs = F.pad(
            features[0], padding_size, value=padding_val
        ).unsqueeze(0)
