...
def remove_duplicate_annotations(data, tol=1e-3):
    # remove duplicate annotations (same category and starting/ending time)
    ants = {'segments': [], 'labels': []}
    for segment, label in zip(data['segments'], data['labels']):
        valid = True
        for p_seg, p_label in zip(ants['segments'], ants['labels']):
            if ((abs(segment[0]-p_seg[0]) <= tol)
                and (abs(segment[1]-p_seg[1]) <= tol)
                and (label == p_label)
            ):
                valid = False
                break
        if valid:
            ants['segments'].append(segment)
            ants['labels'].append(label)
    ants['segments'] = np.stack(ants['segments'])
    ants['labels'] = np.array(ants['labels'])
    return ants


def load_gt_seg(data_list):
    # data_list: id, fps, duration, segments (ndarray), labels (ndarray)
    ...
    for data in data_list:
        ...
        ants = remove_duplicate_annotations(data)
        # video id
        vids += [data['id']] * len(ants['segments'])
        # for each event, grab the start/end time and label
        for segment, label in zip(ants['segments'], ants['labels']):
            starts += [float(segment[0])]
            stops += [float(segment[1])]
            labels += [label]
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.ground_truth = load_gt_seg(data_list)
...
def compute_average_precision_detection(
    ...
    # print(tp)
    # print(tp.shape)
    # print(recall_cumsum.shape)
    # print(recall_cumsum)

    precision_cumsum = tp_cumsum / (tp_cumsum + fp_cumsum)
    print(tp_cumsum)
    print(fp_cumsum)
    print(precision_cumsum)
